...
const sessionContentHashes = new Set();
// ============================================================================
// Public API
// ============================================================================
/**
 ...
 */
export function canAutoCreate() {
    if (sessionAutoCreationCount >= MAX_PER_SESSION)
        return false;
    if (currentMessageCount - lastAutoCreationMessage < MIN_MESSAGE_GAP)
        return false;
    return true;
}
/**
 ...
 */
export function isDuplicateInSession(problem, solution) {
 ...
 */
export function isDuplicateOnDisk(triggers, projectRoot) {
 ...
 */
export function autoCreateSkill(pattern, projectRoot) {
    ...
    if (result.success && result.path) {
        ...
        recordAutoCreation();
        if (DEBUG_ENABLED) {
            console.log('[learner:auto-writer] Auto-created skill: %s (confidence: %d, path: %s)', skillName, pattern.confidence, result.path);
        }
        return result.path;
    }
